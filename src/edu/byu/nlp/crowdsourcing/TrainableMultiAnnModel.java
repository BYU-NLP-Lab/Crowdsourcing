...
public abstract class TrainableMultiAnnModel implements MultiAnnModel {
	private static final Logger logger = Logger.getLogger(TrainableMultiAnnModel.class.getName());
	...
	public void sample(String variableName, String[] args) {
		...
		Preconditions.checkArgument(args.length>=1, "Must specify number of samples. Sampling until convergence is not supported.");
        ...
        logger.info(numSamples+" samples at annealing temp " + temp+" gives LogJoint=" + logJoint());
	}
	...
	public void maximize(String variableName, String[] args) {
	    ...
	    logger.info("Maximization for "+numIterations+" iterations gives LogJoint="+logJoint());
	}
